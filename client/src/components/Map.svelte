<script lang="ts">
	...
	const SCALE = colorScale.length-1;
	const MAX = Math.max(...Object.values(data.byState));
	
    function normalizePopulation(population) {
        return Math.floor(population / (MAX / SCALE));
    }
	...
	<div class="spectrum">
		{#each colorScale as color}
			<div class="color" style="background-color:{color}">
				{Math.floor(colorScale.indexOf(color)*(MAX/SCALE))}+
			</div>
		{/each}
	</div>
